...
poids = ones(1, N) / N; % Poids initial équilibré

% Estimation du mélange de gaussiennes par EM :
moyennes = zeros(1, N);
variances = zeros(1, N);
argument_min_courant = inf;
...
while  ~arret
        ...
        if compteur >= 1
            mu_test = I_max * rand(1, N); % Loi uniforme entre 0 et 255
            sigma_test = 10 + 15 * rand(1, N); % Loi uniforme entre 10 et 25
            [poids,~] = argument_eq_8(mu_test,sigma_test,liste_nvg,F);
        end
	    % Estimation de la probabilité a posteriori
        gaussienne = zeros(length(liste_nvg),N);
	    for q = 1:N
		    mu_q = mu_test(q);
		    sigma_q = sigma_test(q);
		    gaussienne(:,q) = poids(q)/sqrt(2*pi)*exp(-(liste_nvg-mu_q).^2/(2*sigma_q^2))/sigma_q;
        end
        somme_gaussienne = sum(gaussienne,2);
        probas = gaussienne ./ somme_gaussienne;
        % Mise à jour des paramètres
            for k = 1:N
                poids(k) = sum(gaussienne(:, k))/ length(liste_nvg);
                mu_test(k) = sum(gaussienne(:, k) .* liste_nvg') / sum(gaussienne(:, k));
                sigma_test(k) = sqrt(sum(gaussienne(:, k) .* ((liste_nvg' - mu_test(k)).^2)) / sum(gaussienne(:, k)));
            end
            % Calcul de la log-vraisemblance
            log_vraisemblance_old = score;
            log_vraisemblance = 0;
            for i = 1:length(liste_nvg)
                x_i = liste_nvg(i);
                vraisemblance_x_i = 0;
                for k = 1:N
                    mu_k = mu_test(k);
                    sigma_k = sigma_test(k);
                    vraisemblance_x_i = vraisemblance_x_i + poids(k) / sqrt(2 * pi) * exp(-(x_i - mu_k)^2 / (2 * sigma_k^2)) / sigma_k;
                end
                log_vraisemblance = log_vraisemblance + log(vraisemblance_x_i);
            end
             % Calcul du score de convergence
             diff_log_vraisemblance = log_vraisemblance - log_vraisemblance_old;
            difference_score = abs(diff_log_vraisemblance);
            compteur = compteur + 1;
            [poids,argument] = argument_eq_8(mu_test,sigma_test,liste_nvg,F);
	if argument<argument_min_courant
		argument_min_courant = argument;
		moyennes = mu_test;
		...
		pause(0.1);
    hold off;
    if argument_min_courant < arg_min || compteur > compt_max
        arret = true;
    end
end
